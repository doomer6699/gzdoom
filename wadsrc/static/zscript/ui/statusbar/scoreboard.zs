...
extend class BaseStatusBar
{
    ...
    {
        scoreboardFont = ui_classic? SmallFont : NewSmallFont;
    }
    ...
    virtual void Scoreboard_DrawScores(int playerNum, double ticFrac)
    {
	    ...
	    int FontScale = max(screen.GetHeight() / (ui_classic? 200 : 400), 1);
	...
	virtual void Scoreboard_DoDrawScores(PlayerInfo player, Array<int> sortedPlayers, int FontScale, double ticFrac)
	{
		...
		int col2 = (scoreboardFont.StringWidth(text_color) + (ui_classic? 8 : 16)) * FontScale;
		int col3 = col2 + (scoreboardFont.StringWidth(text_frags) + (ui_classic? 8 : 16)) * FontScale;
		int col4 = col3 + maxScoreWidth * FontScale;
		int col5 = col4 + (maxNameWidth + (ui_classic? 8 : 16)) * FontScale;
